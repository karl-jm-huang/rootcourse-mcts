...
def generate_all_time_table(one_of_alltime):
  try:
    ...
    # 读取df
    path = '../data_test3_smooth'# path = '../data_test3_modified'
    abspath = os.path.join(os.path.abspath(path), time+'.csv')
    df = pd.read_csv(abspath, header=None, sep=',', names=['i','e','c','p','l','PV'])
    ...
    # 存储
    # print("-----------------SAVING TABLE FILE---------------")
    np.save(file="../Alltime_smooth_real_PV_table/"+time+".npy", arr=PV_array)
    # print("------------------------SAVED--------------------")
    # return PV_array
  except:
    print('ERRPR',time)

# 对比文件夹获取未处理的时刻，返回list
def get_unfinished_timelist():
  all = get_filename_list_from_dir('../data_test3_modified')
  some = get_filename_list_from_dir('../Alltime_real_PV_table')
  result = list(set(all).difference(set(some)))
  print(result)
  return result
...
if __name__ == "__main__":
  ...
  # abnormal_times = get_abnormal_times_list('../Anomalytime_test3.csv') # [1538176200000,1538464200000,1538388000000,1538402700000] 
  # pool = Pool(12)
  ...
  all_times = get_filename_list_from_dir('../data_test3_smooth')# [1538176200000,1538464200000,1538388000000,1538402700000] 
  # some_times = ['1539429300000','1539636900000','1539649800000','1539670500000','1539845400000','1539957000000','1540054200000','1540094700000','1540122900000','1540374000000','1540469100000','1540470900000','1540510200000','1540114800000','1540331100000','1540498500000']#get_unfinished_timelist()
  pool = Pool(4)
  results2 = pool.map(generate_all_time_table, all_times)
  # results2 = pool.map(generate_all_time_table, some_times)
  end2 = timeit.default_timer()

  # 单进程生成一个时刻PV值表
  # generate_all_time_table(1539778500000)

  # print('multi ABNORMAL:',end0-start0)
  #print('original ABNORMAL:',end1-start1)
  print('multi ALL:',end2-start2)

  # get_filename_list_from_dir('../data_test3')
