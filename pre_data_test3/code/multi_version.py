...
def get_result(time):
    ...
    forecast = np.load(file='../Abnormalytime_forecast_PV_table/' + str(time) + '.npy')  #(150, 15, 10, 37, 6)
    forecast = np.array(forecast)

    real = np.load(file='../Abnormalytime_real_PV_table/' + str(time) + '.npy')
    real = np.array(real)
    ...
    # 下标转换为属性值并且按格式追加进结果文件 result.csv
    transform_and_write_to_csv(time, result_seq)

    print(time, result_seq, result_Q)
...
if __name__ == '__main__':
    ...
    timelist = [1538176200000, 1538230200000, 1538250300000] # filepath_to_time('../Abnormalytime_forecast_PV_table/1538176200000.npy')

    pool = Pool(3)
    start_time = datetime.datetime.now()
    # name, Q = get_result(forecast, real, time)
    results = pool.map(get_result, timelist)
    end_time = datetime.datetime.now()

    print('---------------------------------------------------------------')
    print("异常时刻：")
    print(timelist)# print(sys.argv[3])
    print("运行时间：")
    print(end_time-start_time)
    print ("根因组合: ")
    print (results)# print (json.dumps(name, ensure_ascii=False))#.encode("utf8")
    # print ("组合得分: ")
    # print (Q)
    print('---------------------------------------------------------------')
