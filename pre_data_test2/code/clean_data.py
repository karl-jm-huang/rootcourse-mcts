...
# 删掉字母和unknown,保存到data_test2_modified
def deal_with_char_and_unknown(filepath):
  ...
  save_path = os.path.join('../data_test2_modified',os.path.basename(filepath))
...
# 清洗data_test2目录下的数据，找出各维度的合理属性记录到文件(用0替换unknown)，并且把处理完的新数据放到data_test2_modified目录下
...
# 用于多进程版本的目标函数，清洗data_test2目录下的数据，找出各维度的合理属性记录到文件(用0替换unknown)，并且把处理完的新数据放到data_test2_modified目录下
...
if __name__=='__main__':
  ...
  files = get_file_list("../data_test2")
  pool = Pool(12)
